...
function remote(server, webdriverType) {
  return webdriver.remote(server.host, server.port, server.username, server.password, webdriverType);
}
...
function setup(testname, options) {
  ...
  port = url.parse(options.url).port || 80
  ...
  driver = remote(options.server, options.webdriverType);
  ...
  before(function (done) {
    ...
    function connect() {
      console.log("requesting browser: %s version: %s platform: %s testname: %s", options.browser[0], options.browser[1], options.browser[2], testname);
      driver.init({
        ...
        console.log(options, testname)
        if (err && err.code === 'ECONNREFUSED') {
          var msg = 'Unabled to connect to selenium server at ' + options.server.host + ':' + options.server.port;
          err = new Error(msg);
          err.stack = '';
          console.log("error connecting to selenium server:", err)
          return done(err);
        }
        if (err) {
          console.log("error requesting browser:", err)
          return done(err);
        }
